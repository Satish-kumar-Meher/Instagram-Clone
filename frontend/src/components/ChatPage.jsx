...
//     const { user, suggestedUsers, selectedUser, userProfile } = useSelector(store => store.auth);
...
//             <section className='w-40 md:w-1/4 my-0 border-r border-gray-300 '>
//             <section className='flex items-start justify-start gap-2 my-5 ml-3'>
//             <Avatar className='h-14 w-14'>
//               <AvatarImage src={userProfile?.profilePicture} alt="profilephoto" />
//               <AvatarFallback>CN</AvatarFallback>
//             </Avatar>
//                 <h1 className='font-bold mb-4 px-3 py-3 text-xl'>{user?.username}</h1>
//           </section>
...
import { Input } from './ui/input';
import { Button } from './ui/button';
import { MessageCircleCode, Trash2 } from 'lucide-react'; // Added Trash2 icon for delete functionality
import Messages from './Messages';
import axios from 'axios';
import { setMessages } from '@/redux/chatSlice';
...
const ChatPage = () => {
    ...
    const deleteChatHandler = async (receiverId) => {
        try {
            ...
            if (res.data.success) {
                // Clear messages and deselect the user after deletion
        ...
        <div className='flex ml-[16%] h-screen'>
            <section className='w-40 md:w-1/4 my-0 border-r border-gray-300'>
                <section className='flex items-start justify-start gap-2 my-5 ml-3'>
                    ...
                    <div className="absolute bottom-2 right-[4px] z-1000 w-3 h-3 bg-green-500 rounded-full border border-white"></div>
                ...
                selectedUser ? (
                    <section className='flex-1 border-l border-l-gray-300 flex flex-col h-full'>
                        <div className='flex gap-3 items-center px-3 py-2 border-b border-gray-300 sticky top-0 bg-white z-10'>
                            ...
                            {/* Delete icon for deleting chat */}
                            <Trash2 
                                className="ml-auto cursor-pointer hover:text-red-500" 
                                onClick={() => deleteChatHandler(selectedUser?._id)} 
                            />
                            <p>Delete chat</p>
                        </div>
                        <Messages selectedUser={selectedUser} />
